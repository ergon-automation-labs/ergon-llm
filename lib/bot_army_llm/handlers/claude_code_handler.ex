defmodule BotArmyLlm.Handlers.ClaudeCodeHandler do
  ...
  def handle_complete(message, reply_to) do
    ...
    case validate_claude_code_payload(payload) do
      :ok ->
        process_claude_code(payload, event_id, reply_to)

      {:error, reason} ->
        Logger.warning("Invalid Claude Code payload: #{inspect(reason)}")
        publish_error_reply(reply_to, event_id, reason, "Invalid request payload")
    end
  end
  ...
  defp process_claude_code(payload, event_id, reply_to) do
    ...
    case llm_client.anthropic_passthrough(payload) do
      ...
      {:error, reason} ->
        ...
        publish_error_reply(reply_to, event_id, reason, "LLM request failed")
    end
  end
  ...
  defp publish_error_reply(reply_to, event_id, _reason, message) do
    error_body =
      Jason.encode!(%{
        "error" => %{
          "type" => "internal_error",
